...
$html = ($_GET['form'] == 1) ? generateEntryForm($entry) : generateEntryFormv1($entry);
$pdf->writeHTML($html, true, false, true, false, '');
...
function generateEntryFormv1($dd)
{
	$html = '<table>';
	...
	$html .= '<th width="59%;"><b>MINIMUM PUBLIC HEALTH STANDARDS</b></th>';
	$html .= '<th width="8%;"><b>Yes</b></th>';
	$html .= '<th width="8%;"><b>No</b></th>';
	$html .= '<th width="8%;"><b>N/A</b></th>';
	$html .= '<th width="18%;"><b>Reason why N/A</b></th>';
	$html .= '</tr>';
	$html .= '<tr> <td colspan="5" class="parent-header" style="background-color:#CFD8DC;font-weight:bold">STAFF</td> </tr>';
	$x = 0;
	$result_array = array();
	$style = '';
	foreach ($dd as $key => $data) {
		if ($x == 2) {
			$result_array[] = 'ESTABLISHMENT';
			$style = 'colspan="5" class="parent-header" style="background-color:#CFD8DC;font-weight:bold"';
		}else{
			$style='';
		}
		$result_array[] = $data['requirement'];

		$html .= '<tr nobr="true">';
		$html .= '<td '.$style.'>';

		if ($key == 49559) {
			$html .= 'Avai <td colspan="5" class="parent-header" style="background-color:#CFD8DC;">STAFF</td>labil1ity of designated Safety Officer with the following functions:';
			$html .= '<ol type="a">';
